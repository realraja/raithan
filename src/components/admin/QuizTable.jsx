...
const UserTable = ({ data }) => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: (row)=> row,
        Cell: ({ cell: { value } }) => (
          <img src={'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQIclu8YXkCr64EGkn23I4q_CeL1c5cD1WDug&s'} alt="avatar" className="w-12 h-12 object-cover rounded-full" />
        ),
        className: "max-sm:hidden sm:table-cell",
      },
      {
      ...
      },
      {
        ...
        sortType: (rowA, rowB) => {
          return rowA.original.forCourse.length - rowB.original.forCourse.length;
        },
        className: "max-sm:hidden sm:table-cell",
      },
      {
      ...
      },
      {
        ...
        Cell: ({ cell: { value } }) =>  + value.length,
        sortType: (rowA, rowB) => {
          return rowA.original.usersDone.length - rowB.original.usersDone.length;
        },
      },
      {
        Header: "Published",
      ...
      },
      {
        ...
        Cell: ({ cell: { value } }) => (
          <button
            onClick={() => {setEditQuiz(value); setEditQuizDialog(true)}}
  ...
  const {
  ...
  } = useTable(
    {
      columns,
      data: filteredData,
      initialState: { pageIndex: 0, pageSize: 8, sortBy: [
        {
          id: 'createdAt',
  ...
  return (
    <>
      ...
      {editQuiz && <UpdateQuiz confirmState={editQuizDialog} setConfirmState={(state) => {
