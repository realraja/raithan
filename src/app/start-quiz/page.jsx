...
const QuizCard = ({ quiz, user }) => {
  return (
    <Link href={`/start-quiz/${quiz._id}`} className={quiz.publish || 'hidden'}>
      <div className="border p-4 rounded-lg shadow-md bg-gray-800/75">
        ...
        <div className="flex justify-between items-center mt-4">
          <span className="text-sm text-gray-300">
            Students Done: {quiz.usersDone.length}
          </span>
          
            {quiz.usersDone.find((i) => i.id === user._id) ? (
              ...
              <button className="bg-purple-700 text-white rounded-full p-2 hover:bg-purple-800">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="currentColor"
